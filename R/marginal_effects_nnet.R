...
function(model, 
    ...
    out1 <- lapply(c(vars$nnames, vars$lnames), dydx, data = data, model = model, 
                   type = NULL, eps = eps, ...)
    ...
    for (i in seq_along(vars$fnames)) {
        out2[[i]] <- dydx.factor(data = data, model = model, vars$fnames[i], 
                                 fwrap = (vars$fnames != vars$fnames2)[i], 
                                 type = NULL, ...)
    }
